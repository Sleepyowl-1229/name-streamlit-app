...
def main():
    ...
    if st.button("✨ 開始分析姓名", type="primary", use_container_width=True):
        ...
        with tab2:
            ...
            for char in surname:
                final_s, _, note = get_name_stroke_count(char)
                st.markdown(
                    f'<div class="stroke-item"><b>姓氏</b> 【{char}】：<b>{final_s}</b> 劃 <small style="color:#888">{note}</small></div>',
                    unsafe_allow_html=True,
                )
            for char in given_name:
                final_s, _, note = get_name_stroke_count(char)
                st.markdown(
                    f'<div class="stroke-item"><b>名字</b> 【{char}】：<b>{final_s}</b> 劃 <small style="color:#888">{note}</small></div>',
                    unsafe_allow_html=True,
                )
